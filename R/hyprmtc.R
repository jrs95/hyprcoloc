...
prior = function(p.1, gamma, k){
  if(k==1){p.1}
  else{i=c(2:k);G=(1-gamma^(i-1));p.1*prod(G)}
};
...
#' HyPrMTC
#'
#' hyprmtc is a function which allows the user to perform multi-trait colocalisation analyses in genomic regions
...
#' @param bb.type branch and bound algorithm type
#' @param reg.steps regional step paramter
#' @param window.size size of window for 2CV testing
#' @param sentinel sentinel variant
#' @param epsilon tolerance parameter
...
#' se <- matrix(abs(rnorm(3000)), nrow=1000, ncol=3)
#'
#' # Colocalisation analyses
#' results <- hyprmtc(beta,se)
#' @export
hyprmtc = function(effect.est, effect.se, binary.outcomes = rep(0, dim(effect.est)[2]), trait.subset = c(1:dim(effect.est)[2]), trait.names = c(1:dim(effect.est)[2]), snp.id = c(1:dim(effect.est)[1]), ld.matrix = diag(1, dim(effect.est)[1], dim(effect.est)[1]), trait.cor = diag(1, dim(effect.est)[2], dim(effect.est)[2]), sample.overlap = matrix(rep(1,dim(effect.est)[2]^2), nrow = dim(effect.est)[2]), n.cvs = 1, bb.alg = TRUE, bb.selection = "regional", test.2 = FALSE, reg.steps = 1, window.size = dim(effect.est)[1], sentinel = 0, epsilon = 0, reg.thresh = "default", align.thresh = "default", reg.tol = 0.699, prior.1 = 1e-4, prior.2 = 0.98, prior.3 = 1e-3, prior.4 = 0.995, sensitivity = F, sense.1 = 1, sense.2 = 2, cor.adj.priors = FALSE, uniform.priors = TRUE, branch.jump = FALSE, ind.traits = FALSE){
  ...
  if(length(which(df$NaNs!=0)) > 1){print("Either the Z score correlation matrix or adjusted prior matrix has a high condition number: consider increasing epsilon by a small amount");}
