...
In this example, HyPrMTC identified traits 1-5 as being colocalized with posterion probability $0.999$, regional association probability $0.999$, candidate snp rs11591147 which explains 100\% of the posterior 
...
```{r}
res <- hyprmtc(betas, ses, trait.names=traits, snp.id=rsid, bb.selection = "regional", uniform.priors = FALSE);
res
```
...
```{r}
trait.cor <- diag(1, dim(betas)[2], dim(betas)[2])#hyprmtc::test.betas
ld.matrix <- diag(1, dim(betas)[1], dim(betas)[1])#hyprmtc::test.ses
