...
#### Computing the credible sets of snps for each cluster of colocalized traits 
...
Note that the results 'res' are now a list. The element "res[[1]]" contains the results from before, i.e.
```{r}
res[[1]];
```

The element "res[[2]]" is a list of length 3 containing the snp scores for each of the 3 clusters of colocalized traits (presented in the same order are the clusters in "res[[1]]"), i.e "res[[2]][[1]]" is a vector of length the number of snps in the region and gives a probability that each snp is the causal variant in the region for the cluster of traits 1-5. "res[[2]][[2]]" returns the same information but now for the cluster of putatively colocalized traits 6-8 and "res[[2]][[3]]" for the cluster 9-10. For example the first few snp scores for cluster 1 (traits 1-5) are:

```{r}
head(res[[2]][[1]]);
```

HyPrColoc has an inbuilt function to triage this information down to a "credible set" of snps for each cluster using the output from the "hyprcoloc" function when "snpsocres = TRUE". The default is to identify a credible set of snps which explain 95% of the posterior probability of colocalization for each cluster of colocalized traits. This can be performed using the "cred.set" function, i.e.

```{r}
cred.sets(res, value = 0.95);
```

We see that snp "rs11591147" explains all of the posterior probability of colocalization between the cluster of traits 1-5. This is therefore a strong candidate causal variant in the region. However for the second cluster of traits (6-8), there is no clear candidate causal variante as both "rs12117612" and "rs7532349" explain equal amounts of the posterior probability of colocalization and, as we now see are in perfect LD with one another:

```{r}
ld.matrix["rs12117612","rs7532349"];
```

Together these two variants explain nearly 90% of the posterior probability of colocalization and it is therefore likely* that the traits do colocalize, however it is impossible (using HyPrColoc) to prioritise one of these variants over the other. To prioritise one snp external information would need to be integrated, which can be (e.g.) biological information or by adding another layer to the analysis in which the same trait(s) is measured in a different study. 

*One natural question to ask is, if the two snps are in perfect LD why have we deduced that the traits colocalize. This detail is a result of the influence of the prior probability of colocalization on analyses. The number of causal configurations which reflect colocalization between the traits is much smaller than configurations which do not represent colocalization between the traits. This means that if we were to set all prior configuration probabilities equal we would almost certainly never identify colocalization between the traits, even if the shared associations between traits were truly owing to a colocalization mechanism. Thus a colocalization causal confiugration prior must be larger than a non-colocalization causal configuration prior in general. But how much larger? This is complicated and we therefore provide more detail on this issue later.    
...
```{r}
# 100 traits
# beta100.c1 replicates the betas for cluster 1 (traits 1-5) 100 times etc...  
...
for(c in clusters){
print(length(unlist(strsplit(res$results$traits[c], split=", "))))
}
```
...
Again, for 1000 traits HyPrColoc correctly identified the three clusters of colocalized traits. However, notice that the posterior probability of colocalization has dropped quite a lot for clusters 2 and 3. This is a feature of assessing large numbers of traits jointly in a Bayesian framework, we discuss this nuiance in more detail in the HyPrColoc text. A final point of note is that, the posterior probability explained by the candidate snps in clusters 2 and 3 is maximized. That is, we known from assessing the credible sets of snps in both clusters that in cluster 2 the candidate snp is in perfect LD with one other snp and in cluster 3 the candidate snp is in perfoect LD with four other snps. Hence, the maximum evidential support for these two snps being the causal snps in each cluster can only be 50\% and 20\% respectively, which is what we identify.
