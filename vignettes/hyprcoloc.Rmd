...
We start by loading the package and some data needed to run analyses using HyPrColoc in the first part of this excercise. To execute a basic analysis, users need only input two matrices of equal size: (i) a matrix of effect estimates (betas), with the columns denoting the study traits and rows the snps, and; (ii) a matrix of corresponding standard errors (ses). 
...
For each iteration of the algorithm, HyPrColoc outputs:
...
In this example, HyPrColoc identified traits 1-5 as being colocalized with posterion probability $1$, regional association probability $1$, candidate snp rs11591147 which explains 100\% of the posterior 
...
For technical reasons, analyses have a modest dependence on whether a trait is continuous or binary in the sample. To let HyPrColoc know which traits are continuous (coded 0) or binary (coded 1) we use the 'binary.outcome' variable. For example, suppose the first three traits are binary, then we type 
